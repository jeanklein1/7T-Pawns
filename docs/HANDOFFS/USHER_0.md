...
3. **The boot mood is drawn, not authored — and drawn among the open
   skies**: the seed decides under the destination law's weights, through
   `pick_open_mood` (`shape_is_open` rows only) — the audience wakes
   outdoors; rooms are earned through doors. `DemoConfig.boot_mood` dies;
   `?mood=` stays as the forcing override; `?seed=` now pins the mood
   with the world it pins.
...
Commit: `USHER_0 U3 — the boot mood is drawn from the open skies; DemoConfig sheds boot_mood`
...
- Reload without params, several times: `[World] Boot seed=N (draw)` then
  `[World] Boot mood=<name> (draw)`; only the three skies arrive —
  ≈ sunset 40%, night 30%, noon 30% — **never** a room, the atrium, or
  `finite_outdoor`. The boot card carries `world  mood=…`.
